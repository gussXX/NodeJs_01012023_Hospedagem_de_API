  ...
  try {
    ...
    if (requisition.values.parc.isInstallments == true) {
        ...
        for (let i = 0; i < mounthsList.length; i++) {
          ...
          if (mounthsList[i] == 'December') {
            ...
            for (let index = 0; index < mounthsList.length; index++) {
              mesDaParcela2 = mounthsList[initialMes]
      ...
      for (let index = 0; index < arrayParcelas.length; index++) {
        ...
        const update = { $push: { ['years.' + controllerYear + '.mounths.' + mes]: { $each: [newObject], $position: 0 } } };
        var result = await collection.updateOne(query, update);

        console.log(arrayParcelas)

        if (arrayParcelas[index] == 'December') {
          controllerYear++;
          console.log(controllerYear)
        }
      }
